...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}"
      class="{{ auth()->check() && auth()->user()->theme === 'light' ? 'light' : 'dark' }}">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            ...
            <div class="flex flex-col items-center justify-between gap-4 sm:flex-row">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-sm font-semibold uppercase tracking-[0.25em] {{ auth()->check() && auth()->user()->theme === 'light' ? 'text-indigo-600' : 'text-indigo-300' }}">
                        SelfCheq
                    </span>

                </div>

                <p class="text-xs {{ auth()->check() && auth()->user()->theme === 'light' ? 'text-slate-500' : 'text-slate-400' }}">
                    © 2026 SelfCheq. All rights reserved.
                </p>
        ...
        window.addEventListener('load', () => {
            ...
            navigator.serviceWorker.register('/sw.js')
                .then(registration => {
                    ...
                    console.log(
                        'SelfCheq service worker registered:',
                        registration.scope
                    );
                ...
                .catch(error => {
                    ...
                    console.error(
                        'SelfCheq service worker registration failed:',
                        error
                    );
