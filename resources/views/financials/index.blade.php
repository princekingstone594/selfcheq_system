<x-app-layout>
    <div class="space-y-6">
        ...
        <section class="rounded-3xl border border-rose-400/20 bg-slate-900/70 p-6 shadow-xl">
            ...
            <form method="POST" action="{{ route('financials.store') }}" class="mt-4 space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="frequency" class="mt-1 rounded-2xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-white focus:border-rose-500 focus:ring-1 focus:ring-rose-500">
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 text-sm text-slate-300">
                            <input type="checkbox" name="is_recurring" value="1" class="rounded border-slate-700 bg-slate-800 text-rose-500 focus:ring-rose-500" checked>
                            Recurring
                        </label>
                    </div>
            ...
            <!-- Bills List -->
            @if($bills->where('is_completed', false)->count() > 0)
                <div class="mt-6 space-y-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Active Bills</p>
                    @foreach($bills->where('is_completed', false) as $bill)
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-2 text-xs text-slate-400">
                                        ...
                                        @endif
                                        @if($bill->frequency)
                                            <span class="rounded-full bg-slate-700 px-3 py-1">{{ ucfirst($bill->frequency) }}</span>
                                        @endif
            ...
            @if($bills->where('is_completed', true)->count() > 0)
                <div class="mt-6 space-y-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Paid Bills (History)</p>
                    @foreach($bills->where('is_completed', true) as $bill)
                        <div class="rounded-2xl border border-white/10 bg-slate-800/30 p-4 opacity-60">
                            <div class="flex items-center gap-2">
                                <span class="text-rose-400">📋</span>
                                <p class="font-semibold text-white">{{ $bill->title }}</p>
                                @if($bill->due_date)
                                    <span class="text-xs text-slate-500">— Due: {{ \Carbon\Carbon::parse($bill->due_date)->format('M d, Y') }}</span>
                                @endif
                            </div>
                            @if($bill->amount)
        ...
        <section class="rounded-3xl border border-amber-400/20 bg-slate-900/70 p-6 shadow-xl">
            ...
            <form method="POST" action="{{ route('financials.store') }}" class="mt-4 space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 text-sm text-slate-300">
                            <input type="checkbox" name="is_recurring" value="1" class="rounded border-slate-700 bg-slate-800 text-amber-500 focus:ring-amber-500" checked>
                            Recurring
                        </label>
                    </div>
                        ...
                        <div class="rounded-2xl border border-white/10 bg-slate-800/50 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1">
                                    ...
                                    <div class="mt-2 flex flex-wrap gap-2 text-xs text-slate-400">
                                        @if($tithe->due_date)
                                            <span class="rounded-full bg-slate-700 px-3 py-1">Next: {{ \Carbon\Carbon::parse($tithe->due_date)->format('M d, Y') }}</span>
                                        @endif
            ...
            @if($tithers->where('is_completed', true)->count() > 0)
                <div class="mt-6 space-y-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Completed Tithes (History)</p>
                    @foreach($tithers->where('is_completed', true) as $tithe)
                        <div class="rounded-2xl border border-white/10 bg-slate-800/30 p-4 opacity-60">
                            <div class="flex items-center gap-2">
                                <span class="text-amber-400">🙏</span>
                                <p class="font-semibold text-white">{{ $tithe->title ?: 'Tithe Payment' }}</p>
                                @if($tithe->due_date)
                                    <span class="text-xs text-slate-500">— {{ \Carbon\Carbon::parse($tithe->due_date)->format('M d, Y') }}</span>
                                @endif
                            </div>
                            <div class="mt-2">
                                <form method="POST" action="{{ route('financials.toggle', $tithe) }}" class="inline">
        ...
        <section class="rounded-3xl border border-emerald-400/20 bg-slate-900/70 p-6 shadow-xl">
            ...
            <form method="POST" action="{{ route('financials.store') }}" class="mt-4 space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 text-sm text-slate-300">
                            <input type="checkbox" name="is_recurring" value="1" class="rounded border-slate-700 bg-slate-800 text-emerald-500 focus:ring-emerald-500">
                            Recurring
                        </label>
                    </div>
            ...
            <!-- Savings Plans List -->
            @if($savings->where('is_completed', false)->count() > 0)
                <div class="mt-6 space-y-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Your Savings Plans</p>
                    @foreach($savings->where('is_completed', false) as $saving)
                        <div class="rounded-2xl border border-white/10 bg-slate-800/50 p-4">
                            <div class="flex items-start justify-between gap-3">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2">
            ...
            @if($savings->where('is_completed', true)->count() > 0)
                <div class="mt-6 space-y-3">
                    <p class="text-xs font-semibold uppercase tracking-wider text-slate-400">Completed Goals (History)</p>
                    @foreach($savings->where('is_completed', true) as $saving)
                        <div class="rounded-2xl border border-white/10 bg-slate-800/30 p-4 opacity-60">
                            <div class="flex items-center gap-2">
                                <span class="text-emerald-400">💰</span>
                                <p class="font-semibold text-white">{{ $saving->title }}</p>
